...
const FileProcessingVisaDescriptionPage = () => {
  const [fileprocessingRequirements, setfileprocessingRequirements] = useState([]);
  const [fileprocessingtravelitinerary, setfileprocessingtravelitinerary] = useState([]);
    ...
    const fetchfileprocessingData = async () => {
      try {
        ...
        const { data } = await axios.get(
          `/api/fileprocessingapi/fileprocessing/${params.countryId}`
        );
  ...
  return (
    <div className="lg:pt-[120px] antialiased">
      <div className="h-[400px] flex flex-col rounded-3xl mx-[35px] bg-cover bg-center bg-fixed bg-no-repeat mb-10 relative" style={{ backgroundImage: `url(${bgurl})` }} >
...
export default FileProcessingVisaDescriptionPage;
