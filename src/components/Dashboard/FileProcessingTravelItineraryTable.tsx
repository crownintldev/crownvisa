...
interface FileProcessingTravelItinerary {
  key: string;
  id: number;
  title: string;
  description: string;
...
const FileProcessingTravelItineraryTable: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const fileprocessingResponse = await axios.get("/api/fileprocessingapi/fileprocessing");
        ...
        fileprocessingTravelItineraryResponse.data.fileprocessingtravelItinerary;

        const transformedData = fileprocessingTravelItineraryData.map(
          (fileprocessingtravelitinerary: FileProcessingTravelItinerary) => {
            const Title = fileprocessingData.find(
              (fileprocessing: FileProcessing) => fileprocessing.id === fileprocessingtravelitinerary.countryid
            );
            return {
              key: fileprocessingtravelitinerary.id,
              title: fileprocessingtravelitinerary.title,
              description: fileprocessingtravelitinerary.description,
  ...
  const handleConfirmDelete = async () => {
    ...
    await axios.delete(`/api/fileprocessingapi/fileprocessingtravelitinerary/${selectedId}`);
      ...
      <Layout style={{ marginLeft: siderWidth }}>
        ...
        <Content style={contentStyle}>
          ...
          <Drawer
            title={
              formType === "edit"
                ? "Edit fileprocessing Travel Itinerary"
                : "Add fileprocessing Travel Itinerary"
            }
            placement={placement}
            closable={false}
            onClose={onClose}
            ...
            width={600}
          >
            {formType === "edit" && <FileProcessingTravelItinerary id={selectedId} />}
            {formType === "add" && <FileProcessingTravelItinerary addid={countryid} />}
          </Drawer>
        </Content>
      </Layout>
    </Layout>
...
export default FileProcessingTravelItineraryTable;
